...
public class UserDataGridPreferencesServiceImpl  {
    ...
    public void saveMultiGridPreferences( MultiGridPreferencesStore multiGridPreferencesStore ) {
        final Path preferencesPath = userServicesBackend.buildPath( identity.getIdentifier(), "multi-datagrid-preferences", multiGridPreferencesStore.getMultiGridId() );
        try {
            ioServiceConfig.startBatch( preferencesPath.getFileSystem() );
            ioServiceConfig.write( preferencesPath, xs.toXML( multiGridPreferencesStore ) );
        } finally {
            ioServiceConfig.endBatch();
        }
    }
    ...
    public MultiGridPreferencesStore loadMultiGridPreferences( String key ) {
        Path preferencesPath = userServicesBackend.buildPath( identity.getIdentifier(), "multi-datagrid-preferences", key );
        try {
            if ( ioServiceConfig.exists( preferencesPath ) ) {
                final String xml = ioServiceConfig.readAllString( preferencesPath );
                return (MultiGridPreferencesStore) xs.fromXML( xml );
            }
        } catch ( final Exception e ) {
        }
        return null;
