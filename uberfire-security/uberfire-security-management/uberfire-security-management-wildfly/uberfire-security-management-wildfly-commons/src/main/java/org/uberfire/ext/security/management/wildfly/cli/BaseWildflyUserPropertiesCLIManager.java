 ...
 */
public abstract class BaseWildflyUserPropertiesCLIManager extends BaseWildflyCLIManager implements UserManager, ContextualManager {

    private static final Logger LOG = LoggerFactory.getLogger(BaseWildflyUserPropertiesCLIManager.class);
    private BaseWildflyUserPropertiesManager usersPropertiesManager;

    public BaseWildflyUserPropertiesCLIManager() {
        this( new ConfigProperties( System.getProperties() ) );
    }

    public BaseWildflyUserPropertiesCLIManager(final Map<String, String> gitPrefs) {
        this( new ConfigProperties( gitPrefs ) );
    }

    public BaseWildflyUserPropertiesCLIManager(final ConfigProperties gitPrefs) {
        loadConfig( gitPrefs );
    }

    private String getUsersPropertiesFilePath() throws Exception {
        return getPropertiesFilePath("authentication");
    }

    protected abstract BaseWildflyUserPropertiesManager getWildflyUserPropertiesManager(Map<String, String> arguments, UserSystemManager usManager);
    ...
    private void init(final UserSystemManager usManager)  {
        try {
            ...
            this.usersPropertiesManager = getWildflyUserPropertiesManager(arguments, usManager);
        } catch (Exception e) {
