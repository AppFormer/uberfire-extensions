 ...
 */
public abstract class BaseWildflyGroupPropertiesCLIManager extends BaseWildflyCLIManager implements GroupManager, ContextualManager {

    private static final Logger LOG = LoggerFactory.getLogger(BaseWildflyGroupPropertiesCLIManager.class);
    protected BaseWildflyGroupPropertiesManager groupsPropertiesManager;

    public BaseWildflyGroupPropertiesCLIManager() {
        this( new ConfigProperties( System.getProperties() ) );
    }

    public BaseWildflyGroupPropertiesCLIManager(final Map<String, String> gitPrefs) {
        this( new ConfigProperties( gitPrefs ) );
    }

    public BaseWildflyGroupPropertiesCLIManager(final ConfigProperties gitPrefs) {
        loadConfig( gitPrefs );
    }

    protected abstract BaseWildflyGroupPropertiesManager createWildflyGroupPropertiesManager(Map<String, String> arguments);

    public BaseWildflyGroupPropertiesManager getGroupsPropertiesManager() {
        return groupsPropertiesManager;
    }

    protected  String getGroupsPropertiesFilePath() throws Exception {
        return getPropertiesFilePath("authorization");
    }
    ...
    private void init()  {
        try {
            ...
            this.groupsPropertiesManager = createWildflyGroupPropertiesManager(arguments);
        } catch (Exception e) {
