...
/**
 * <p>Base (abstract) users manager service provider implementation for JBoss WildFly, when using default realm based on properties files.
 * Implementation classes for different WildFly versions can use this as a base class.</p>
 */
public abstract class BaseWildflyUserPropertiesManager extends BaseWildflyPropertiesManager implements UserManager, ContextualManager {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(BaseWildflyUserPropertiesManager.class);
    ...
    WildflyPropertiesFileLoader usersFileLoader;

    public BaseWildflyUserPropertiesManager() {
        this( new ConfigProperties( System.getProperties() ) );
    }

    public BaseWildflyUserPropertiesManager(final Map<String, String> gitPrefs) {
        this( new ConfigProperties( gitPrefs ) );
    }

    public BaseWildflyUserPropertiesManager(final ConfigProperties gitPrefs) {
        loadConfig( gitPrefs );
    }
    ...
    public synchronized BaseWildflyGroupPropertiesManager getGroupsPropertiesManager() {
        try {
            return (BaseWildflyGroupPropertiesManager) userSystemManager.groups();
